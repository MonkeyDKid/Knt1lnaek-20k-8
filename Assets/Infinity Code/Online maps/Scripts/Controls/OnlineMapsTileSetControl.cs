...
public class OnlineMapsTileSetControl : OnlineMapsControlBase3D
{
    ...
    /// <summary>
    /// This event is occurs when the vertices of the marker is generated.
    /// </summary>
    ...
    /// <summary>
    /// This event is called when an error occurred when downloading the new elevations.
    /// </summary>
    public Action<string> OnElevationFails;

    /// <summary>
    /// This event is called when a new elevation value requested.
    /// </summary>
    ...
    /// <summary>
    /// This event occurs when marker mesh generation is completed.
    /// </summary>
    ...
    /// <summary>
    /// Cancels the current request for downloading elevation data.
    /// </summary>
    public void CancelCurrentElevationRequest()
    {
    ...
    public override float GetBestElevationYScale(Vector2 topLeftPosition, Vector2 bottomRightPosition)
    {
        ...
        realDistance.x = realDistance.x / map.tilesetSize.x * map.width;
        realDistance.y = realDistance.y / map.tilesetSize.y * map.height;
    ...
    public override float GetBestElevationYScale(double tlx, double tly, double brx, double bry)
    {
        ...
        dx = dx / map.tilesetSize.x * map.width;
        dy = dy / map.tilesetSize.y * map.height;
    ...
    /// <summary>
    /// Starts downloading of the new elevation data for the current area.
    /// </summary>
    ...
    public Vector3 GetWorldPositionWithElevation(Vector2 coords, Vector2 topLeftPosition, Vector2 bottomRightPosition)
    {
        ...
        float y = GetElevationValue(px, pz, GetBestElevationYScale(topLeftPosition, bottomRightPosition), topLeftPosition, bottomRightPosition);
    ...
    private void InitMapMesh()
    {
        ...
        bool isIgnore = ignoreGetElevation;
        ignoreGetElevation = true;
        UpdateMapMesh();
        ignoreGetElevation = isIgnore;
    }
    ...
    protected override void OnDestroyLate()
    {
        ...
        OnElevationRequested = null;
        OnElevationFails = null;
    ...
    private void UpdateMapSubMesh(int x, int y, int w, int h, double subMeshSizeX, double subMeshSizeY, int subMeshVX, int subMeshVZ, double startPosX, double startPosZ, float yScale, double tlx, double tly, double brx, double bry, Material[] materials, ref float minY, ref float maxY, bool hasTraffic, bool hasOverlayBack, bool hasOverlayBackAlpha, bool hasOverlayFront, bool hasOverlayFrontAlpha)
    {
        ...
        if (currentTile != null)
        {
            ...
            if (!hasTileTexture)
            {
                ...
                {
                    tileTexture = OnlineMapsTile.emptyColorTexture = new Texture2D(1, 1, TextureFormat.ARGB32, mipmapForTiles);
