...
public class OnlineMapsDrawingElement: IOnlineMapsInteractiveElement
{
    ...
    protected List<Vector2> GetLocalPoints(IEnumerable points, bool closed = false, bool optimize = true)
    {
        double sx, sy;
        OnlineMaps map = api;
        int mapZoom = map.buffer.apiZoom;
        OnlineMapsProjection projection = map.projection;
        projection.CoordinatesToTile(tlx, tly, mapZoom, out sx, out sy);
        ...
        while (enumerator.MoveNext())
        {
            ...
            {
                Vector2 point = (Vector2)p;
                projection.CoordinatesToTile(point.x, point.y, mapZoom, out px, out py);
                useValue = true;
            }
            else if (valueType == 3)
            {
            ...
            {
                if (valueType == 1) projection.CoordinatesToTile((float)v2, (float)v1, mapZoom, out px, out py);
                else if (valueType == 2) projection.CoordinatesToTile((double)v2, (double)v1, mapZoom, out px, out py);
                useValue = true;
            }
            ...
            if (i == 0)
            {
                double ox = px - map.tilesetWidth / OnlineMapsUtils.tileSize / 2;
                if (ox < -halfMaxX) px += maxX;
                else if (ox > halfMaxX) px -= maxX;
            }
            else
            {
                double ox = px - ppx;
    ...
    protected void InitLineMesh(IEnumerable points, OnlineMapsTileSetControl control, ref List<Vector3> vertices, ref List<Vector3> normals, ref List<int> triangles, ref List<Vector2> uv, float width, bool closed = false, bool optimize = true)
    {
        ...
        int maxX = 1 << api.zoom;
        float maxSize = maxX * OnlineMapsUtils.tileSize * api.tilesetSize.x / api.tilesetWidth;
