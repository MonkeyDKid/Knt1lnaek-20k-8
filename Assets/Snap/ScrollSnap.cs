...
public class ScrollSnap : UIBehaviour, IDragHandler, IEndDragHandler {
	...
	void LerpToElement() {
		...
		float newX = Mathf.Lerp(releasedPosition.x, targetPosition.x-(50*cellIndex), t);
		content.anchoredPosition = new Vector2(newX, content.anchoredPosition.y);		
		print(content.anchoredPosition.x);
	}
	...
	void SetContentSize(int elementCount) {
		content.sizeDelta = new Vector2((cellSize.x+50) * elementCount, content.rect.height);
	}
	...
	bool ShouldStopLerping() {
		return Mathf.Abs(content.anchoredPosition.x+(50*cellIndex) - targetPosition.x) < 0.001;;
	}
}
